...
class Client {
  ...
  apiCall (endPoint, data) {
    if (!this.auth) throw new Error('Not configured - Please provide an access token or an API key')

    let headers = { 'Content-Type': 'application/json' }

    return post(`https://${config.host}/v1/${endPoint}.json`, JSON.stringify(data || {}), { ...this.options['headers'], headers })
  }
}
...
let retrieveTokens = function (data) {
  ...
  return post(`https://${config.host}/oauth/token`, querystring.stringify(data || {}), headers)
}
